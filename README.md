...
Authentication uses OIDC (Workload Identity Federation) — no client secrets. When using an existing Foundry project, the CI/CD service principal needs the **Azure AI User** role (minimum) and **Azure AI Developer** role on the Foundry resource for agent deployment — see the [Infrastructure Guide](docs/infrastructure-guide.md#rbac-roles) for setup instructions.
